...
*/
  ...
  function createFilterRoot(header) {
  ...
  function headerNeedsEnhance(header) {
  ...
  function ensureSearchChromeFresh() {
    if (!search_chrome_isSearchPage()) return;
    const header = getHeader();
    if (!header) {
      scheduleEnhance();
  ...
  function hookHistoryNavigation(onNavigate) {
    const notify = () => {
